...
const orderRu = [
  { id: 1, title: 'Ихрам', icon: '🤍' },
  { id: 2, title: 'Таваф', icon: '🕋' },
  { id: 3, title: 'Сай', icon: '🏃' },
  { id: 4, title: 'Арафат', icon: '⛰️' },
  { id: 5, title: 'Муздалифа', icon: '🌙' },
  { id: 6, title: 'Бросание камней', icon: '🪨' },
  { id: 7, title: 'Жертвоприношение', icon: '🐑' },
]
...
export function KazhylykOrderGame() {
  ...
  const correctCount = checked ? sequence.filter((item, i) => item.id === i + 1).length : 0

  const ui = {
    back: lang === 'ru' ? '← Хадж' : '← Қажылық',
    title: lang === 'ru' ? '🧭 Составь путь хаджа' : '🧭 Қажылық жолын құрастыр',
    subtitle: lang === 'ru' ? 'Расставь шаги в правильном порядке' : 'Қадамдарды дұрыс ретімен орналастыр',
    yourOrder: lang === 'ru' ? 'Твой порядок' : 'Сенің реттілігің',
    pickCard: lang === 'ru' ? 'Выбери карточку снизу' : 'Төменнен карточка тап',
    youPlaced: lang === 'ru' ? 'Вы поставили:' : 'Сіз қойдыңыз:',
    check: lang === 'ru' ? '✅ ПРОВЕРИТЬ' : '✅ ТЕКСЕРУ',
    result: lang === 'ru' ? 'РЕЗУЛЬТАТ' : 'НӘТИЖЕ',
    placed: lang === 'ru' ? 'шагов расставлено правильно' : 'дұрыс орналастырылды',
      ...
      <main translate="no" className="min-h-screen bg-gradient-to-br from-teal-50 to-cyan-50 flex items-center justify-center px-4 py-8">
        <div className="max-w-md w-full">
          <div className="bg-white rounded-3xl shadow-xl border border-teal-200 px-6 py-8 text-center">
            ...
            <p className="text-5xl font-bold text-teal-600 mb-2">{correctCount}/{correctOrder.length}</p>
            <p className="text-gray-400 text-sm mb-6">{ui.placed}</p>
            <div className="space-y-2 mb-6 text-left">
              {correctOrder.map((step, i) => {
                const userStep = sequence[i]
                const ok = userStep?.id === step.id
                return (
                  <div key={step.id} className={`rounded-xl p-2.5 border flex items-center gap-3 text-sm ${ok ? 'bg-teal-50 border-teal-200' : 'bg-red-50 border-red-200'}`}>
                    <span className="text-xl flex-shrink-0">{step.icon}</span>
                    <div className="flex-1">
                      <p className="font-medium text-gray-800">{i + 1}. {step.title}</p>
                      {!ok && userStep && <p className="text-xs text-red-500">{ui.youPlaced} {userStep.title}</p>}
                    </div>
                    <span>{ok ? '✅' : '❌'}</span>
                  </div>
                )
            ...
            <div className="flex flex-col gap-3">
              <button onClick={restart} className="w-full bg-gradient-to-r from-teal-500 to-cyan-600 text-white font-bold py-3 rounded-2xl shadow hover:shadow-lg transition-all hover:scale-105">{ui.replay}</button>
    ...
    <main translate="no" className="min-h-screen bg-gradient-to-br from-teal-50 to-cyan-50 px-4 py-6">
      <div className="max-w-lg mx-auto">
        <div className="flex items-center justify-between mb-4">
          <Link to="/kazhylyk" className="text-sm text-teal-700 hover:text-teal-900 transition-colors">{ui.back}</Link>
          <span className="text-sm text-gray-500 font-medium">{sequence.length}/{correctOrder.length}</span>
        </div>
        ...
        <div className="bg-white border-2 border-dashed border-teal-300 rounded-2xl p-4 mb-4 min-h-20">
          <p className="text-xs text-gray-400 mb-3 font-medium uppercase tracking-wider text-center">{ui.yourOrder}</p>
          {sequence.length === 0 ? (
            ...
            <div className="flex flex-col gap-2">
              {sequence.map((item, i) => {
                ...
                return (
                  <button key={item.id} onClick={() => removeFromSequence(item)}
                    className={`flex items-center gap-3 rounded-xl px-4 py-2.5 border-2 text-sm font-semibold transition-all ${isCorrect ? 'bg-teal-50 border-teal-400 text-teal-800' : isWrong ? 'bg-red-50 border-red-400 text-red-800' : 'bg-teal-50 border-teal-200 text-gray-800 hover:border-teal-400'}`}>
        ...
        {sequence.length === correctOrder.length && !checked && (
          <button onClick={check} className="w-full bg-gradient-to-r from-teal-500 to-cyan-600 text-white font-bold py-4 rounded-2xl shadow-md hover:shadow-lg transition-all hover:scale-105 active:scale-95">
            {ui.check}
          </button>
        )}
      </div>
