...
interface Pair { id: number; name: string; image: string }
...
const pairsRu: Pair[] = [
  { id: 1, name: 'Такбир', image: '/images/takbir.png' },
  { id: 2, name: 'Кийам', image: '/images/qyam-qyraat.png' },
  { id: 3, name: 'Руку', image: '/images/ruku.png' },
  { id: 4, name: 'Саджда', image: '/images/sajda.png' },
  { id: 5, name: 'Ташаhhуд', image: '/images/tashahud.png' },
  { id: 6, name: 'Салам', image: '/images/salam-right.png' },
]
...
export function ImageMatchGame() {
  ...
  const t = {
    back: lang === 'ru' ? '← Намаз' : '← Намаз',
    title: lang === 'ru' ? 'Игра — Картинки' : 'Ойын — Сурет-атау жұптастыру',
    subtitle: lang === 'ru' ? 'Выбери картинку слева, затем нажми её название справа' : 'Сол жақтан суретті таңда, оң жақтан оның атауын басып байланыстыр',
    hintSelected: lang === 'ru' ? 'Картинка выбрана — нажми название справа' : 'Сурет таңдалды — оң жақтан атауын баса байланыстыр',
    hintDefault: lang === 'ru' ? 'Начни с выбора картинки слева' : 'Сол жақтан суретті таңдаудан бастаңыз',
    imgCol: lang === 'ru' ? 'Картинки' : 'Суреттер',
    nameCol: lang === 'ru' ? 'Названия' : 'Атаулар',
    checkBtn: lang === 'ru' ? 'Проверить' : 'Тексеру',
    connectAll: (c: number, t: number) => lang === 'ru' ? `Соедини все (${c} / ${t})` : `Барлығын байланыстыр (${c} / ${t})`,
    allCorrect: lang === 'ru' ? 'МашаАллах! Все картинки угаданы!' : 'МашаАллаh! Барлық суретті дұрыс таптың!',
    good: lang === 'ru' ? 'Хорошо! Повтори до 100%.' : 'Жақсы! Қайта байқап 100% жеткіз.',
    tryAgain: lang === 'ru' ? 'Перечитай движения и попробуй снова!' : 'Қимылдарды оқып қайта байқа!',
    ...
    ariaLabel: (id: number) => lang === 'ru' ? `Картинка ${id}` : `${id}-сурет`,
  }
  ...
  return (
    <main translate="no" className="max-w-2xl mx-auto px-4 py-8">
      <div className="mb-6">
        <Link to="/namaz" className="text-sm text-primary-600 hover:underline">{t.back}</Link>
        <h1 className="text-2xl md:text-3xl font-bold text-primary-900 mt-2">{t.title}</h1>
        <p className="text-gray-500 mt-1 text-sm">{t.subtitle}</p>
      </div>

      <div className={`mb-4 rounded-xl px-4 py-2.5 text-sm text-center font-medium transition-all duration-200 ${selectedLeft !== null ? 'bg-primary-50 border border-primary-200 text-primary-800' : 'bg-gray-50 border border-gray-200 text-gray-400'}`}>
        {selectedLeft !== null ? t.hintSelected : t.hintDefault}
      </div>
      ...
      <div ref={containerRef} className="relative">
        <svg className="absolute inset-0 pointer-events-none" style={{ width: '100%', height: '100%', overflow: 'visible' }} aria-hidden="true">
          <defs>
            {lines.map((l) => (
              <marker key={`dot-img-${l.leftId}`} id={`dot-img-${l.leftId}`} markerWidth="6" markerHeight="6" refX="3" refY="3">
          ...
          {lines.map((l) => {
            ...
            return <path key={l.leftId} d={`M ${l.x1} ${l.y1} C ${mx} ${l.y1}, ${mx} ${l.y2}, ${l.x2} ${l.y2}`} fill="none" stroke={stroke} strokeWidth={2.5} strokeLinecap="round" opacity={0.85} markerStart={`url(#dot-img-${l.leftId})`} markerEnd={`url(#dot-img-${l.leftId})`} />
          })}
        </svg>

        <div className="flex gap-6">
          <div className="flex-1 flex flex-col gap-3">
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1 text-center">{t.imgCol}</p>
            {pairs.map((pair) => {
              ...
              return (
                <button key={pair.id} ref={(el) => { leftRefs.current[pair.id] = el }} onClick={() => handleLeftClick(pair.id)} disabled={checked} aria-label={checked ? pair.name : t.ariaLabel(pair.id)}
                  className={['relative flex items-center justify-center rounded-xl border-2 w-full h-24 transition-all duration-150', 'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-400', isSelected ? 'border-primary-500 bg-primary-50 shadow-md scale-[1.02]' : isCorrect ? 'border-green-400 bg-green-50' : isWrong ? 'border-red-400 bg-red-50' : isConnected ? 'border-gray-300 bg-white' : 'border-gray-200 bg-white hover:border-gray-300 hover:shadow-sm'].join(' ')}>
                  <img src={pair.image} alt={checked ? pair.name : ''} className="max-w-full max-h-full object-contain p-2" />
                  {isConnected && !checked && <span className="absolute bottom-1.5 right-1.5 w-2.5 h-2.5 rounded-full" style={{ backgroundColor: color }} />}
                  {checked && <span className={`absolute top-1.5 right-1.5 text-base font-bold ${isCorrect ? 'text-green-500' : 'text-red-400'}`}>{isCorrect ? '✓' : '✗'}</span>}
                </button>
              )
            })}
          </div>
          ...
          <div className="flex-1 flex flex-col gap-3">
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1 text-center">{t.nameCol}</p>
            {shuffledRight.map((rightId) => {
              ...
              return (
                <button key={rightId} ref={(el) => { rightRefs.current[rightId] = el }} onClick={() => handleRightClick(rightId)} disabled={checked}
                  className={['flex items-center justify-center gap-2 rounded-xl border-2 px-3 py-2 w-full h-24 transition-all duration-150', 'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-400', isCorrect ? 'border-green-400 bg-green-50' : isWrong ? 'border-red-400 bg-red-50' : isConnected ? 'border-gray-300 bg-white' : selectedLeft !== null ? 'border-gray-200 bg-white hover:border-primary-300 hover:bg-primary-50 hover:shadow-sm' : 'border-gray-200 bg-white'].join(' ')}>
                  {isConnected && !checked && <span className="w-2.5 h-2.5 rounded-full flex-shrink-0" style={{ backgroundColor: color }} />}
                  <span className="text-sm font-semibold text-gray-800 text-center leading-snug">{pair.name}</span>
